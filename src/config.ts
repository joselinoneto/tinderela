...
export const TRAVEL_HEURISTICS = {
  ...
  /**
   * Fixed minutes per stop: approach, docking, kiosk. Cargo handling is NOT
   * modelled — how long a load takes depends on auto-load, container sizes and
   * the player's own hauling, so the tools report the loading conditions at
   * each terminal (`auto_load`) and let the player judge instead of inventing
   * a per-SCU rate.
   */
